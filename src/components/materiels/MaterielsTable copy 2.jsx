...
const MaterielsTable = () => {
  ...
  useEffect(() => {
    const loadFilterData = async () => {
      try {
        ...
        const [beneficiairesRes, achatsRes, fournisseursRes] = await Promise.all([
          getAllBeneficiaires(),
          getAllAchats(),
          getAllFournisseurs()
        ]);
  ...
  const filteredFournisseurs = fournisseurs.filter(fournisseur => {
    ...
    return (
      ...
      (fournisseur.telephone && fournisseur.telephone.toLowerCase().includes(searchLower)) ||
      (fournisseur.matriculeFiscale && fournisseur.matriculeFiscale.toLowerCase().includes(searchLower))
    );
  });

  // Charger les matériels avec tous les filtres
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Application des filtres par priorité
        if (selectedBeneficiaire) {
          ...
          // Si aucun filtre spécifique, appliquer les filtres d'état
          if (selectedEtat === 'disponibles') {
            response = await getMaterielsDisponibles();
          } else if (selectedEtat === 'attribues') {
        ...
        // Appliquer le filtre fournisseur côté client si sélectionné
    ...
    fetchData();
  }, [activeTab, selectedBeneficiaire, selectedAchat, selectedFournisseur, selectedEtat]);
        ...
        <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
          <button 
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${
              activeTab === 'all' && !selectedBeneficiaire && !selectedAchat && selectedEtat === 'all'
        ...
        <div className="bg-white rounded-xl shadow-md p-4 mb-6 border border-gray-200">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-semibold text-gray-700 flex items-center">
              <FiSliders className="mr-2" /> Filtres avancés
            </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <h3 className="font-semibold text-gray-800 mb-3 flex items-center">
                <span className="bg-purple-100 text-purple-800 w-8 h-8 rounded-full flex items-center justify-center mr-2">1</span>
                État du matériel
              </h3>
              ...
              <div className="flex flex-wrap gap-2">
                {etatOptions.map(option => (
                  <button
                    key={option.value}
                    onClick={() => {
                      setSelectedEtat(option.value);
                      if (option.value === 'disponibles') setActiveTab('disponibles');
                      else if (option.value === 'attribues') setActiveTab('attribues');
                      else setActiveTab('all');
                    }}
                    className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center ${
                      selectedEtat === option.value
                        ? option.value === 'all' 
                          ? 'bg-gray-600 text-white'
                          : option.value === 'DISPONIBLE' || option.value === 'disponibles'
                            ? 'bg-green-600 text-white'
                            : option.value === 'ATTRIBUE' || option.value === 'attribues'
                              ? 'bg-blue-600 text-white'
            ...
            <div className="space-y-3">
              <h3 className="font-semibold text-gray-800 mb-3 flex items-center">
                <span className="bg-purple-100 text-purple-800 w-8 h-8 rounded-full flex items-center justify-center mr-2">2</span>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div>
                      {selectedFournisseur ? (
                        <>
                          <div className="font-medium text-gray-800">{selectedFournisseur.nom}</div>
                          <div className="text-sm text-gray-600">
                            {selectedFournisseur.email || 'Email non spécifié'}
                          </div>
                        </>
                      ) : (
                        <span className="text-gray-500">Sélectionner un fournisseur...</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center">
                    ...
                    <FiChevronDown className={`transition-transform ${showFournisseurDropdown ? 'rotate-180' : ''}`} />
                  </div>
                </button>
                        ...
                        filteredFournisseurs.map(fournisseur => (
                          <div
                            ...
                            className={`px-4 py-3 hover:bg-purple-50 cursor-pointer border-b border-gray-100 last:border-b-0 ${
                              selectedFournisseur?.id === fournisseur.id ? 'bg-purple-50' : ''
                            }`}
                          >
                            ...
                            <div className="text-sm text-gray-600 flex justify-between mt-1">
                              <span>{fournisseur.email || 'Email N/A'}</span>
                              <span>{fournisseur.telephone || 'Tél N/A'}</span>
                            </div>
                            <div className="text-xs text-gray-500 mt-1 truncate">
                              {fournisseur.matriculeFiscale ? `MF: ${fournisseur.matriculeFiscale}` : ''}
                            </div>
                          </div>
                        ))
                      )}
              ...
              {selectedFournisseur && (
                <div className="mt-3 p-3 bg-purple-50 border border-purple-200 rounded-lg">
                  <div className="flex items-center">
                    ...
                    <div>
                      <span className="font-medium text-purple-800">Fournisseur sélectionné:</span>
                      <div className="text-sm text-purple-700">
                        {selectedFournisseur.nom} • {selectedFournisseur.email || 'Email non spécifié'}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-3">
              <h3 className="font-semibold text-gray-800 mb-3 flex items-center">
                <span className="bg-purple-100 text-purple-800 w-8 h-8 rounded-full flex items-center justify-center mr-2">3</span>
              ...
              <div className="relative">
                ...
                >
                  <div className="flex items-center">
                    <FiUser className="mr-3 text-purple-500" />
                    <div>
                      {selectedBeneficiaire ? (
                        <>
                          <div className="font-medium text-gray-800">
                      ...
                      {loadingFilters ? (
                        ...
                        filteredBeneficiaires.map(beneficiaire => (
                          <div
                            ...
                            className={`px-4 py-3 hover:bg-purple-50 cursor-pointer border-b border-gray-100 last:border-b-0 ${
                              selectedBeneficiaire?.id === beneficiaire.id ? 'bg-purple-50' : ''
                            }`}
                          >
                            <div className="font-medium text-gray-800">
              ...
              {selectedBeneficiaire && (
                <div className="mt-3 p-3 bg-purple-50 border border-purple-200 rounded-lg">
                  <div className="flex items-center">
                    <FiCheck className="text-purple-500 mr-2" />
                    <div>
                      <span className="font-medium text-purple-800">Bénéficiaire sélectionné:</span>
                      <div className="text-sm text-purple-700">
                        {selectedBeneficiaire.nom} {selectedBeneficiaire.prenom} • 
                        {selectedBeneficiaire.matricule ? ` ${selectedBeneficiaire.matricule} •` : ''}
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                >
                  <div className="flex items-center">
                    <FiShoppingCart className="mr-3 text-purple-500" />
                    <div>
                      {selectedAchat ? (
                        <>
                      ...
                      {loadingFilters ? (
                        ...
                        filteredAchats.map(achat => (
                          <div
                            ...
                            className={`px-4 py-3 hover:bg-purple-50 cursor-pointer border-b border-gray-100 last:border-b-0 ${
                              selectedAchat?.id === achat.id ? 'bg-purple-50' : ''
                            }`}
                          >
                            <div className="font-medium text-gray-800">
              ...
              {selectedAchat && (
                <div className="mt-3 p-3 bg-purple-50 border border-purple-200 rounded-lg">
                  <div className="flex items-center">
                    <FiCheck className="text-purple-500 mr-2" />
                    <div>
                      <span className="font-medium text-purple-800">Achat sélectionné:</span>
                      <div className="text-sm text-purple-700">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              <div className="mt-1 text-gray-600">
                                {materiel.fournisseur?.nom || materiel.prix?.fournisseur?.nom || materiel.achat?.fournisseur?.nom || 'Non spécifié'}
                              </div>
