...
const MaterielsTable = () => {
  ...
  const [exercices, setExercices] = useState([]); // Nouvel état pour les exercices
  ...
  const [selectedExercice, setSelectedExercice] = useState(null); // Nouveau filtre exercice
  ...
  const [showExerciceDropdown, setShowExerciceDropdown] = useState(false); // Nouveau dropdown exercice
  ...
  const filteredExercices = exercices.filter(e => {
    ...
    return e && e.toLowerCase().includes(searchLower);
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (selectedExercice) {
          filteredData = filteredData.filter(m => {
            return m.exercice === selectedExercice;
          });
        }
    ...
    fetchData();
  }, [selectedBeneficiaire, selectedAchat, selectedFournisseur, selectedType, selectedMarque, selectedSysteme, selectedExercice, selectedEtat]);
  ...
  const resetFilters = () => {
    ...
    setSelectedExercice(null); // Reset exercice
    setSelectedEtat('all');
    setSearchTerm('');
    ...
    setMarqueSearch('');
    setSystemeSearch('');
    setExerciceSearch(''); // Reset recherche exercice
  ...
  const getActiveFiltersCount = () => {
    ...
    if (selectedExercice) count++; // Ajout du compteur pour exercice
          ...
          {getActiveFiltersCount() > 0 && (
            <div className="mt-6 pt-4 border-t border-gray-200">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {/* Affichage du filtre exercice actif */}
                {selectedExercice && (
                  <span className="px-3 py-1.5 bg-purple-100 text-purple-800 rounded-full text-xs font-medium flex items-center gap-1">
                    <FiCalendar size={12} />
